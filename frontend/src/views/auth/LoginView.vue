...
<template>
    ...
    <div class="rounded-2xl bg-white shadow-lg ring-1 ring-gray-900/5 p-8">
        ...
        <form @submit.prevent="handleLogin" class="space-y-5">
            <!-- General error -->
            <div v-if="generalError" class="rounded-lg bg-red-50 border border-red-200 p-3 text-sm text-red-700">
                {{ generalError }}
            </div>
            ...
            <!-- Submit -->
            <button type="submit" :disabled="authStore.isLoading" :class="[
                ...
                <span :class="{ 'opacity-0': authStore.isLoading }">Đăng nhập</span>
                <!-- Loading spinner -->
                <span v-if="authStore.isLoading" class="absolute inset-0 flex items-center justify-center">
    ...
    <p class="text-center text-sm text-gray-500">
        Chưa có tài khoản?
        <router-link to="/auth/register" class="font-semibold text-indigo-600 hover:text-indigo-500 transition-colors">
            Đăng ký ngay
        </router-link>
    </p>
</template>
